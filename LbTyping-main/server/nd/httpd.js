    ...
    app.use((req, res, next) => {
        ...
        const getResponseByQid = (qid) => {
            ...
            doQuery(q, r => {
                ...
                const XLSX = require('xlsx')//エクセルファイルを読み込む
        ...
        if (u.pathname == '' || u.pathname == '/' || u.pathname == '/1') {
            ...
            if (u.pathname == "/top.html") {
				...
				else if (u.pathname == "/kekka.html") {
                const Fs = require('fs')
                const html = Fs.readFileSync("user/kekka.html")
                res.end(html)
            }
                ...
                res.end(html)
			}/*追加したelse if*/else if (u.pathname == '/kekka') {
            res.writeHead(200, {'Content-Type': 'text/html; charset=UTF-8'})
            const Fs = require('fs')
            const html = Fs.readFileSync("kekka.html")
            res.end(html)
        }
